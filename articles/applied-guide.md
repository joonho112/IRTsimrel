...
(2026), which treats reliability as a design parameter rather than an
...
| 3 | [`eqc_calibrate()`](https://joonho112.github.io/IRTsimrel/reference/eqc_calibrate.md) | Find $`c^*`$ such that $`\rho(c^*) \approx \rho^*`$ |
...
average-information metric ($`\tilde{\rho}`$) and the MSEM-based metric
($`\bar{w}`$). If your target falls outside the reported range, you must
change the design (e.g., add items or switch to a 2PL model).
...
target <- 0.85
in_range <- identical(feas$target_status_info, "feasible")
...
![Line plot of achievable reliability ranges by test length with the
target reliability shown as a horizontal reference
line.](applied-guide_files/figure-html/step1-feas-plot-1.png)
...
![Reliability curve showing how reliability changes with the
discrimination scaling factor for the selected
design.](applied-guide_files/figure-html/step2-rho-curve-1.png)

The plot shows both $`\tilde{\rho}`$ (average-information) and
$`\bar{w}`$ (MSEM-based). Note that $`\tilde{\rho} \geq \bar{w}`$ holds
on the same population/grid basis by Jensen’s inequality.
...
EQC (Empirical Quadrature Calibration) is Algorithm 1 from Lee (2026).
...
- **Targeting $`\bar{w}`$** directly (the MSEM-based
  reciprocal-information metric).
...
![SAC convergence diagnostic plot for the warm-start validation
run.](applied-guide_files/figure-html/step4-sac-plot-1.png)
...
![Histogram of total scores from the simulated response
matrix.](applied-guide_files/figure-html/step5-score-hist-1.png)
...
![Scatter plot of item difficulty against observed proportion correct in
the simulated
data.](applied-guide_files/figure-html/step5-item-analysis-1.png)
...
> reliability is higher than WLE reliability, but `EAPrel()` and
> `WLErel()` use different estimators and variance bases. Inspect both
> as complementary external diagnostics rather than formal lower/upper
> bounds.
...
| **Monotonicity** | Stable on practical search intervals; finite-grid saturation triggers an interior-maximum check | Not required |
...
1.  **Start with EQC** using `reliability_metric = "info"`. This is fast
    and deterministic.
2.  **Use SAC with `reliability_metric = "info"` to validate EQC** when
    you want a stochastic same-estimand check.
3.  **Use SAC with `reliability_metric = "msem"` separately** when your
    research question targets $`\bar{w}`$ directly.
4.  **Always warm-start SAC** from EQC for faster convergence when
    comparing to an EQC result.
...
the average test information across the latent distribution.

**Properties:**

- Well behaved over practical EQC search intervals; for very wide finite
  empirical quadrature bounds, the implementation checks an interior
  maximum before declaring infeasibility.
...
information varies substantially (e.g., short tests, non-normal
populations).
...
|:---|:---|:---|
| Standard simulation study | `"info"` | Stable on practical intervals; fast deterministic root-finding |
| MSEM-based marginal reliability needed | `"msem"` via SAC | Uses reciprocal-information error variance |
| Non-normal latent distribution | `"info"` for EQC | Bracket-checked deterministic root-finding |
| EQC/SAC validation | `"info"` for both | Compare the same estimand via [`compare_eqc_sac()`](https://joonho112.github.io/IRTsimrel/reference/compare_eqc_sac.md) |
...
calibrated discriminations are $`\lambda_i^* = c^* \times 1 = c^*`$ for
all items.
...
![Bar plot comparing achieved reliability across latent distribution
shapes.](applied-guide_files/figure-html/shape-barplot-1.png)
...
![Reliability curves comparing how test length affects the
discrimination scale needed for a target
reliability.](applied-guide_files/figure-html/test-length-curves-1.png)
...
- If targeting `"msem"`, switch to `"info"` (the bracket-checked EQC
  metric).
...
> simulation framework of Lee (2026), implemented in the R package
...
> Lee, 2026) with a Monte Carlo quadrature sample of size $`M =`$
> \[10,000\] and the average-information reliability metric
> ($`\tilde{\rho}`$). The target marginal reliability was set to
...
> Calibration algorithm (SAC; Algorithm 2 in Lee, 2026) with \[300\]
...
> examinees using the calibrated item parameters. Each response
> $`Y_{pi}`$ was drawn from $`\text{Bernoulli}(p_{pi})`$ where
> $`p_{pi} = \text{logit}^{-1}[\lambda_i^*
> (\theta_p - \beta_i)]`$. Simulation provenance, including metric,
> calibration status, item source, latent shape, and seed, was retained
> with each generated data object.
...
- [`vignette("item-parameters")`](https://joonho112.github.io/IRTsimrel/articles/item-parameters.md)
  ...
  including the copula method and optional IRW integration.
- [`vignette("theory-reliability")`](https://joonho112.github.io/IRTsimrel/articles/theory-reliability.md)
  — Mathematical foundations of $`\tilde{\rho}`$ and $`\bar{w}`$.
