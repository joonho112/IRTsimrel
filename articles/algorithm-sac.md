...
**Positioning**: SAC serves both as the direct MSEM-targeting algorithm
and as a validation companion to EQC. When validating EQC, set
`reliability_metric = "info"` so both algorithms target the same
estimand. SAC also provides trajectory-based convergence diagnostics
that EQC lacks.
...
**Key references**: Lee (2026, arXiv:2512.16012v2), Section 2.5; Robbins
...
where $`\hat{\rho}_n`$ is computed from fresh Monte Carlo samples at
each step, and $`\varepsilon_n`$ is zero-mean noise with bounded
variance: $`\mathbb{E}[\varepsilon_n \mid c_n] = 0`$ and
...
where $`\Pi_{[c_L, c_U]}[\cdot]`$ denotes projection onto the feasible
interval $`[c_L, c_U]`$ and $`a_n`$ is a decreasing step size sequence.

**Intuition**: When $`\hat{\rho}_n(c_n) > \rho^*`$ (current reliability
is too high), the update decreases $`c_n`$ (reducing discrimination,
hence reliability). When $`\hat{\rho}_n(c_n) < \rho^*`$, the update
increases $`c_n`$.
...
**First condition** ($`\Sigma a_n = \infty`$): Ensures the algorithm can
reach any point in the parameter space, regardless of the initial value.
Without this, the total adjustment
$`\sum a_n \cdot |\hat{\rho}_n - \rho^*|`$ might be bounded, preventing
convergence from a distant starting point.

**Second condition** ($`\Sigma a_n^2 < \infty`$): Controls the
accumulated noise. The total noise variance is proportional to
$`\sum a_n^2 \sigma^2_\varepsilon`$, which must be finite for
convergence.
...
**$`a > 0`$ (scale constant)**: Controls the overall magnitude of
updates. Larger $`a`$ means more aggressive steps, leading to faster
initial progress but potentially more oscillation.

In the package: `step_params$a` (default: 1).

**$`A \geq 0`$ (stabilization constant)**: Dampens early iterations
where step sizes would otherwise be very large
($`a_1 = a/(1+A)^\gamma`$). When $`A > 0`$, the effective step size
starts smaller, providing stability during the initial transient phase.
This is particularly important when `c_init` is far from $`c^*`$.

In the package, `step_params$A` defaults to 50. The `burn_in` argument
controls which iterations enter the Polyak-Ruppert average; it does not
set `A` automatically.

**$`\gamma \in (1/2, 1]`$ (decay exponent)**: Controls how quickly step
sizes decrease. In the package: `step_params$gamma` (default: 0.67).
...
![Multiple SAC trajectories comparing how step-size decay changes
convergence behavior over
iterations.](algorithm-sac_files/figure-html/step-size-sensitivity-1.png)
...
- Faster decay ($`\gamma = 1.0`$) converges quickly when starting near
  the solution, but risks under-correcting from a distant start.
...
The raw SAC iterates $`c_n`$ converge at rate $`O(n^{-\gamma})`$, which
is suboptimal. Polyak and Juditsky (1992) showed that a simple average
of the iterates achieves the optimal rate $`O(n^{-1/2})`$, regardless of
$`\gamma`$ (as long as the Robbins-Monro conditions hold).

### The Averaging Formula

After discarding a burn-in period of $`B`$ iterations, the
Polyak-Ruppert average is:
...
The practical benefit is substantial. With $`\gamma = 0.67`$ (the
package default), the final iterate converges at rate $`O(n^{-0.67})`$,
while the Polyak-Ruppert average converges at rate $`O(n^{-0.5})`$ —
nearly the same exponent but with considerably smaller constants due to
variance reduction.
...
The initial value $`c_0`$ significantly affects SAC’s convergence speed.
...
**Definition (APC Formula).** Under the assumptions
$`\theta \sim N(0, 1)`$ and $`\beta_i \sim N(0, \sigma^2_\beta)`$ with
$`\lambda_i^{(0)} = 1`$ (Rasch), the approximate initial value is:
...
where $`\sigma^2 = 1 + \sigma^2_\beta`$ and the logistic-normal
...
$`\mathbb{E}[\lambda^2 p(1-p)] = c^2 \kappa(\sigma^2)`$. With $`I`$
items, the average test information is approximately $`I c^2 \kappa`$,
so $`\tilde{\rho} \approx \sigma^2_\theta I c^2 \kappa /
(\sigma^2_\theta I c^2 \kappa + 1)`$. Setting this equal to $`\rho^*`$
and solving for $`c`$ yields Eq. (7).
...
![Line plot comparing SAC scaling trajectories from EQC warm start, APC
start, and user-specified
starts.](algorithm-sac_files/figure-html/init-comparison-1.png)
...
1.  The step sizes satisfy Eq. (4): $`\sum a_n = \infty`$,
    $`\sum a_n^2 < \infty`$.
...
5.  The noise $`\varepsilon_n`$ satisfies
    $`\mathbb{E}[\varepsilon_n \mid c_n] = 0`$ and
    $`\mathbb{E}[\varepsilon_n^2 \mid c_n] \leq \sigma^2_\varepsilon < \infty`$.
...
2.  **Drift condition**: Since $`g(c)(c - c^*) > 0`$ for $`c \neq c^*`$
    (the reliability function is increasing, so $`g`$ has the same sign
    as $`c - c^*`$), the drift term $`-2a_n(c_n - c^*)g(c_n) < 0`$
    provides contraction toward $`c^*`$.

3.  **Noise control**: The accumulated noise
    $`\sum a_n^2 \sigma^2_\varepsilon <
    \infty`$ (by the step size conditions), so the noise does not
    prevent convergence.

4.  **Almost sure convergence**: By the Robbins-Siegmund theorem (a
    supermartingale convergence result), $`V(c_n) \to V^*`$ a.s. for
    some random variable $`V^*`$. The drift condition then forces
    $`V^* = 0`$, giving $`c_n \to c^*`$ a.s.
...
indicate that the iterates have not yet settled around the solution. The
threshold is relative to the calibrated scale because the diagnostic is
measured on the $`c`$ scale; $`\epsilon`$ is machine precision.
...
When both algorithms use the same reliability metric and comparable
design inputs, and the SAC run has converged, agreement within 5% is
typical and expected.
[`compare_eqc_sac()`](https://joonho112.github.io/IRTsimrel/reference/compare_eqc_sac.md)
checks target, metric, model, item count, and status fields, but it
cannot prove that every latent/item generator argument or resampling
...
1.  **Different reliability metrics**: For EQC/SAC agreement checks,
    ensure SAC uses `reliability_metric = "info"`. Direct SAC `"msem"`
    targeting is a different estimand, so disagreement with EQC-info is
    expected.
...
![SAC diagnostic plot showing scaling factor values across stochastic
approximation
...
![SAC diagnostic plot showing reliability estimates fluctuating around
the target over
iterations.](algorithm-sac_files/figure-html/rho-trajectory-plot-1.png)
