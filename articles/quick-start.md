...
**Prerequisites:** Only the IRTsimrel package is needed. The executable
chunks in the walkthrough run on any system with the current IRTsimrel
version installed; the final full-workflow block is shown as a copyable
template.
...
it. The `target_status_info` and `target_status_msem` fields summarize
this metric-specific screening result.
...
This plot shows both the average-information metric ($`\tilde{\rho}`$,
...
average-information metric above the MSEM-based
metric.](quick-start_files/figure-html/rho-curve-1.png)

Reliability curve for a 20-item Rasch test. On the same quadrature grid,
the average-information metric (blue) lies at or above the MSEM-based
metric (red) due to Jensen’s inequality.
...
- `provenance`: calibration and simulation metadata, including metric,
  status, item source, and seed
...
![Bar plot of item proportion correct ordered by item
difficulty.](quick-start_files/figure-html/item-difficulty-plot-1.png)
...
With `newdata`, it computes reliability at each specified value of
$`c`$:
...
substantially less reliable, while at $`c = 2.0`$ it is more reliable
than needed.
...
When validating an EQC result, set `reliability_metric = "info"` so both
algorithms solve the same calibration problem; agreement is then strong
...
#>   EQC c*              : 1.018304
#>   SAC c*              : 1.039125
#>   Absolute difference : 0.020821
#>   Percent difference  : 2.04%
#>   EQC achieved rho    : 0.8000
#>   SAC achieved rho    : 0.7898
#>   Agreement (< 5%)    : YES
...
![Two-panel SAC convergence plot showing scaling factor and reliability
trajectories across
iterations.](quick-start_files/figure-html/sac-plot-1.png)
...
- **[`vignette("applied-guide")`](https://joonho112.github.io/IRTsimrel/articles/applied-guide.md)**:
  ...
  distributions, optional IRW-based item sources, and factorial
  simulation designs with multiple reliability levels.
...
- **[`vignette("theory-reliability")`](https://joonho112.github.io/IRTsimrel/articles/theory-reliability.md)**:
  ...
  ($`\tilde{\rho}`$ and $`\bar{w}`$), Jensen’s inequality, and the
