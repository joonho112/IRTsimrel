...
and
[`vignette("simulation-design")`](https://joonho112.github.io/IRTsimrel/articles/simulation-design.md)
for background.
...
Without reliability control, any observed difference in recovery
...
#>   2PL: achievable rho_tilde = [0.062, 0.987]
```
...
#> 1 rasch        0.6    0.6000048 0.5754772
#> 2   2pl        0.6    0.5999992 0.5060864
#> 3 rasch        0.7    0.6999999 0.7380681
#> 4   2pl        0.7    0.7000059 0.6511525
#> 5 rasch        0.8    0.8000008 1.0183044
#> 6   2pl        0.8    0.8000001 0.9025704
```
...
$${\widehat{\beta}}_{i} = - \log\left( \frac{p_{i}}{1 - p_{i}} \right)$$

where $p_{i}$ is the proportion of correct responses for item $i$. This
is the standard item calibration under the Rasch model in the limit of
large $N$.
...
parameter recovery. Error bars show the range across
replications.](case-studies_files/figure-html/cs1-plot-1.png)
...
models is attributable to the structural difference between Rasch and
2PL data-generating processes—not to differing data quality. Several
patterns emerge:

- **Higher reliability leads to smaller RMSE** for both models, as
  expected. Better data quality (higher signal-to-noise ratio) improves
  parameter recovery.
...
- **Model differences are modest** when reliability is held constant.
  ...
  difficulty recovery, but much of that apparent difference vanishes
  once reliability is equalized.

- **The critical role of controlling reliability**: the differences
  within a model across reliability levels are typically larger than the
  differences between models at a fixed reliability level.
...
> comprised $I = 20$ items with normally distributed difficulties. For
> each condition, $R = 500$ replications of $N = 1,000$ simulees were
> generated. Item difficulties were estimated via maximum marginal
> likelihood and evaluated via bias and RMSE. The maximum calibration
> error across conditions was less than 0.001.
...
under another, the achieved reliability will differ from the target
because the test information profile interacts with the latent
distribution.
...
parameters. This uses
[`compute_rho_tilde()`](https://joonho112.github.io/IRTsimrel/reference/compute_rho_bar.md)
with theta samples from each distribution, providing the theoretical
prediction of how reliability shifts.
...
#> Reliability under each generating distribution:
print(rho_table)
...
#> heavy_tail heavy_tail        0.8     0.8001    0.0001
```
...
function $\mathcal{J}\left( \theta;c^{*} \right)$ is not uniform across
$\theta$, so different latent distributions weight the information
differently.
...
![Impact of latent distribution misspecification. Top: actual
reliability shifts away from the 0.80 target under non-normal
generation. Bottom: difficulty RMSE varies with generating shape, driven
partly by the reliability
shift.](case-studies_files/figure-html/cs2-plot-1.png)
...
1.  **Reliability shifts**: the actual reliability deviates from the
    target (sometimes higher, sometimes lower), depending on how the
    test information profile aligns with the new distribution.

2.  **Recovery accuracy changes**: RMSE is partly driven by the
    reliability shift and partly by the distribution’s effect on the
    information profile at each ability level.
...
- **Positively skewed**: more examinees are located in the left tail
  where item information may be lower for items centered near
  $\beta = 0$.
...
> calibrated item parameters to ${\widetilde{\rho}}^{*} = 0.80$ under a
> normal latent distribution using EQC ($M = 10,000$; $I = 20$ Rasch
> items). Response data were then generated under four distributions:
> normal, positively skewed (Gamma, $k = 4$), bimodal ($\delta = 0.8$),
> and heavy-tailed (Student-t, $df = 5$), all pre-standardized to mean 0
> and variance 1. For each generating condition, $R = 500$ replications
> of $N = 1,000$ simulees were generated. We report both the theoretical
> reliability under each generating distribution and the difficulty
> recovery RMSE.
...
plus/minus one standard deviation across
replications.](case-studies_files/figure-html/cs3-plot-1.png)
...
approximately as $O\left( 1/\sqrt{N} \right)$. We can check this by
examining the ratio of RMSE values:
...
sample sizes, it confirms the expected $O\left( 1/\sqrt{N} \right)$
scaling.
...
- **N = 200**: a reasonable minimum for stable difficulty recovery at
  $\widetilde{\rho} = 0.80$ with 20 Rasch items. RMSE stabilizes at a
  tolerable level for most applications.

- **N \>= 500**: marginal improvement is small relative to the increased
  data collection cost. Recommended only for high-precision
  applications.

For studies that require difficulty recovery within a specified
...
> To determine the minimum sample size for stable item difficulty
> recovery, we calibrated a 20-item Rasch test to
> ${\widetilde{\rho}}^{*} = 0.80$ using EQC. Response data were
> generated for $N \in \{ 50,100,200,500,1000\}$ simulees across
> $R = 500$ replications per condition. Item difficulties were estimated
> via maximum marginal likelihood and evaluated by RMSE. Diminishing
> returns in RMSE were observed beyond $N = 200$, which we recommend as
> the minimum sample size for this configuration.
...
2.  **Select the IRT model(s)** appropriate for your context. Use
    `"rasch"` for tests with equal discriminations and `"2pl"` for tests
    with varying discriminations.
...
4.  **Set target reliability levels** that span the range relevant to
    your application. Include at least three levels to detect nonlinear
    effects.

5.  **Screen all design cells** for feasibility using
...
6.  **Determine the number of replications.** For published work,
    $R \geq 500$ is standard. For pilot studies, $R = 100$ may suffice.
...
7.  Report feasibility screening results (number screened, number
    infeasible).
...
**Principle 1: Always control reliability.** Without it, model
comparisons are confounded by data quality differences. Case Study 1
demonstrates how reliability control isolates the effect of model
...
at a fixed reliability.

**Principle 2: Document distribution mismatches.** If the calibration
and generating distributions differ, report the actual achieved
reliability under the generating distribution. Case Study 2 shows that
misspecification shifts both reliability and recovery accuracy in
predictable but important ways.
...
choosing sample size. Case Study 3 demonstrates diminishing returns
beyond a minimum threshold and confirms the theoretical
$O\left( 1/\sqrt{N} \right)$ scaling.

**Principle 4: Start with feasibility screening.** Before committing to
a large simulation, use
[`check_feasibility()`](https://joonho112.github.io/IRTsimrel/reference/check_feasibility.md)
and
[`rho_curve()`](https://joonho112.github.io/IRTsimrel/reference/rho_curve.md)
to verify that your design is achievable. Infeasible cells waste
computational resources and produce misleading boundary solutions.
