...
packages. The log-odds estimator is:
...
![Grouped plot of difficulty RMSE by model and target reliability with
error bars across
...
> reliability using the IRTsimrel R package (Lee, 2026). For each of six
...
> reliability using EQC with $`M = 5{,}000`$ quadrature points. Tests
> comprised $`I = 20`$ items with normally distributed difficulties. For
> each condition, this executable demonstration used $`R = 5`$
> replications of $`N = 300`$ simulees; production studies should
> increase both values. Item difficulties were estimated with a fast
> log-odds estimator and evaluated via bias and RMSE. The maximum
> calibration error across conditions was less than 0.001.
...
function $`\mathcal{J}(\theta; c^*)`$ is not uniform across $`\theta`$,
so different latent distributions weight the information differently.
...
![Two-panel bar plot showing actual reliability and difficulty RMSE by
generating latent shape.](case-studies_files/figure-html/cs2-plot-1.png)
...
- **Bimodal**: two modes create regions of low density near
  $`\theta = 0`$ where most items provide maximal information,
  potentially reducing effective information utilization.
...
> latent distribution using EQC ($`M = 10{,}000`$; $`I = 20`$ Rasch
> items). Response data were then generated under four distributions:
> normal, positively skewed (Gamma, $`k = 4`$), bimodal
> ($`\delta = 0.8`$), and heavy-tailed (Student-t, $`df = 5`$), all
> pre-standardized to mean 0 and variance 1. For each generating
> condition, $`R = 500`$ replications of $`N = 1{,}000`$ simulees were
> generated. We report both the theoretical reliability under each
> generating distribution and the difficulty recovery RMSE.
...
![Line plot showing difficulty RMSE decreasing as sample size increases
with a shaded variability
band.](case-studies_files/figure-html/cs3-plot-1.png)
...
- **N = 200**: a reasonable minimum for stable difficulty recovery at
  $`\tilde{\rho} = 0.80`$ with 20 Rasch items. RMSE stabilizes at a
...
> $`\tilde{\rho}^* = 0.80`$ using EQC. Response data were generated for
> $`N \in \{50, 100, 200, 500\}`$ simulees across $`R = 5`$
> demonstration replications per condition. Item difficulties were
> estimated with a fast log-odds estimator and evaluated by RMSE.
...
6.  **Determine the number of replications.** For published work,
    $`R \geq 500`$ is standard. For pilot studies, $`R = 100`$ may
    suffice.
...
for (i in seq_len(nrow(design))) {
  ...
  design$feasible[i] <- feas$target_status_info %in%
                         c("feasible", "boundary_lower", "boundary_upper")
}
...
1.  State IRTsimrel version and cite Lee (2026).
