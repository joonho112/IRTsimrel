...
1.  **Realistic difficulties**: Parametric defaults for portable
    simulations, with optional Item Response Warehouse (IRW) integration
    for empirically grounded difficulty distributions when the IRW
...
[`sim_item_params()`](https://joonho112.github.io/IRTsimrel/reference/sim_item_params.md)
handles this by default using the **copula method** with
$`\rho = -0.3`$.
...
$`\log(\lambda)`$ is close to the default target of $`-0.3`$.
...
IRW is an optional external source for empirically grounded simulations.
It can provide difficulty distributions based on real assessment items
when the IRW package is installed. The default, dependency-free source
remains `source = "parametric"`.
...
![Side-by-side plots comparing parametric item difficulty and
discrimination
distributions.](item-parameters_files/figure-html/parametric-compare-1.png)
...
`model = "2pl"`, you can choose how discriminations are generated. The
default is `method = "copula"`, and three methods are available.
...
> normal errors. When empirical or other non-normal difficulties are
> used, achieved correlations may differ from targets.
...
![Scatter plot of item difficulty and discrimination with diagnostic
reference patterns.](item-parameters_files/figure-html/plot-items-1.png)
...
![Density plots of simulated item difficulty and discrimination
parameters.](item-parameters_files/figure-html/plot-density-1.png)
...
![Scatter plot of calibrated 2PL item difficulties and scaled
discriminations.](item-parameters_files/figure-html/coef-2pl-plot-1.png)
...
Zhang, L., Liu, Y., Molenaar, D., & Domingue, B. (2025). Realistic
Simulation of Item Difficulties. *PsyArXiv*.
