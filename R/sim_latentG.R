...
#'
#' In the Rasch, 2PL, and 3PL models, the latent distribution \eqn{G} affects:
#' \itemize{
...
sim_latentG <- function(n,
  ...
  if (!is.numeric(sigma) || length(sigma) != 1L ||
    ...
    stop("`sigma` must be a positive finite scalar.")
  }
  ...
  if (shape == "floor") {
    ...
    if (!is.numeric(w_floor) || length(w_floor) != 1L ||
        is.na(w_floor) || !is.finite(w_floor) ||
        w_floor <= 0 || w_floor >= 1) {
      stop("`shape_params$w_floor` must be a finite numeric scalar in (0, 1).")
    }
    if (!is.numeric(m_floor) || length(m_floor) != 1L ||
        is.na(m_floor) || !is.finite(m_floor)) {
      stop("`shape_params$m_floor` must be a finite numeric scalar.")
    }
  }
  if (shape == "ceiling") {
    w_ceil <- shape_params$w_ceil %||% 0.3
    m_ceil <- shape_params$m_ceil %||% 1.5
    if (!is.numeric(w_ceil) || length(w_ceil) != 1L ||
        is.na(w_ceil) || !is.finite(w_ceil) ||
        w_ceil <= 0 || w_ceil >= 1) {
      stop("`shape_params$w_ceil` must be a finite numeric scalar in (0, 1).")
    }
    if (!is.numeric(m_ceil) || length(m_ceil) != 1L ||
        is.na(m_ceil) || !is.finite(m_ceil)) {
      stop("`shape_params$m_ceil` must be a finite numeric scalar.")
    }
  }
  if (!is.logical(standardize_custom) || length(standardize_custom) != 1L ||
      is.na(standardize_custom)) {
    stop("`standardize_custom` must be TRUE or FALSE.")
  }
  if (!is.logical(return_z) || length(return_z) != 1L || is.na(return_z)) {
    stop("`return_z` must be TRUE or FALSE.")
  }
  ...
  if (shape == "custom") {
    ...
    if (!is.numeric(weights) || length(weights) == 0L ||
      ...
      stop("`mixture_spec$weights` must be a finite non-negative numeric vector.")
    }
    ...
    if (!is.numeric(means) || length(means) != K ||
      ...
      stop("`mixture_spec$means` must be a finite numeric vector matching weights.")
    }
    if (!is.numeric(sds) || length(sds) != K ||
        anyNA(sds) || any(!is.finite(sds)) || any(sds <= 0)) {
      stop("`mixture_spec$sds` must be a positive finite numeric vector matching weights.")
    }
    if (length(means) != K || length(sds) != K) {
...
#'
#' @export
print.latent_G <- function(x, digits = 4, ...) {
  cat("Latent Ability Distribution (G-family)\n")
  cat("=======================================\n")
  cat(sprintf("  Shape     : %s\n", x$shape))
  cat(sprintf("  n         : %d\n", x$n))
  cat(sprintf("  Target mu : %.*f\n", digits, x$mu))
  cat(sprintf("  Target sigma: %.*f\n", digits, x$sigma))
  cat("\nSample Moments:\n")
  cat(sprintf("  Mean      : %.*f\n", digits, x$sample_moments$mean))
  cat(sprintf("  SD        : %.*f\n", digits, x$sample_moments$sd))
  cat(sprintf("  Skewness  : %.*f\n", digits, x$sample_moments$skewness))
  cat(sprintf("  Kurtosis  : %.*f (excess)\n", digits, x$sample_moments$kurtosis))
  invisible(x)
}
...
plot.latent_G <- function(x,
  ...
  if (!is.numeric(bins) || length(bins) != 1L ||
      is.na(bins) || !is.finite(bins) || bins <= 0 || bins %% 1 != 0) {
    stop("`bins` must be a positive integer scalar.")
  }
...
#' @rdname as.numeric.latent_G
#' @export
as.double.latent_G <- function(x, ...) {
