...
- `provenance`:
  ...
  List describing the calibration result and simulation settings used to
  generate the response data, including result class, target and
  achieved reliability, metric, model, test length, scalar
  `calibration_status`, character-vector `status_flags`, item
  source/design, calibration call, simulation seed, sample size, latent
  shape, and latent parameters.
