...
- reliability_metric:
  ...
  :   Average-information reliability (default, recommended for EQC).
      Targets \\\tilde{\rho}\\, which is well behaved on practical EQC
...
- `misc`:
  ...
  List of root-finding diagnostics, including bounds, endpoint
  reliabilities, target status, and the
  [`uniroot()`](https://rdrr.io/r/stats/uniroot.html) result when an
  interior root was used.
...
- **Average-information** (`"info"`/`"tilde"`, **default**): Uses the
  ...
  \rho^\*\\ is stable on practical search intervals. For very wide
  finite empirical quadrature bounds, the implementation checks for an
  interior maximum before declaring a target infeasible.

**MSEM-based** (`"msem"`/`"bar"`) reliability uses the harmonic mean of
test information, \\\bar{w}(c) = \sigma^2\_\theta / (\sigma^2\_\theta +
E\[1/\mathcal{J}(\theta;c)\])\\. This metric is rejected by
`eqc_calibrate()` because the objective \\\bar{w}(c) - \rho^\*\\ may be
non-monotone (see the reliability-targeted simulation manuscript,
Section 4.3), which can cause single-bracket root-finding to return an
incorrect solution. Use
[`sac_calibrate`](https://joonho112.github.io/IRTsimrel/reference/sac_calibrate.md)
if you need to target \\\bar{w}\\ directly.

### WLE vs EAP Reliability Interpretation

When validating with TAM, EAP reliability is often greater than WLE
reliability in practical fits, but the two coefficients use different
estimators and variance bases. EAP reliability more directly corresponds
to MSEM-based population reliability. Treat WLE and EAP as complementary
external diagnostics rather than guaranteed lower/upper bounds.
