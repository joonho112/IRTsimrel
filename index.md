...
- **Scale** — A single global scaling factor $c^{*}$ is calibrated so
  that $\lambda_{i}^{*} = c^{*} \cdot \lambda_{i,0}$ yields the target
  reliability.
...
|-----------|---------------------------------------------------------------------------------------|---------------------------------------------|---------------------------------|
| **EQC**   | [`eqc_calibrate()`](https://joonho112.github.io/IRTsimrel/reference/eqc_calibrate.md) | Deterministic root-finding (Brent’s method) | Default: fast, exact, no tuning |
| **SAC**   | [`sac_calibrate()`](https://joonho112.github.io/IRTsimrel/reference/sac_calibrate.md) | Stochastic approximation (Robbins–Monro)    | Cross-validation, complex DGPs  |
...
|---------------------|--------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------|
| **Internal**        | [`predict()`](https://rdrr.io/r/stats/predict.html), [`compute_rho_both()`](https://joonho112.github.io/IRTsimrel/reference/compute_rho_both.md) | Consistency of calibrated reliability; Jensen’s gap between $\widetilde{\rho}$ and $\bar{w}$ |
| **Cross-algorithm** | [`compare_eqc_sac()`](https://joonho112.github.io/IRTsimrel/reference/compare_eqc_sac.md)                                                        | Agreement between EQC and SAC on $c^{*}$                                                     |
...
- [`vignette("case-studies")`](https://joonho112.github.io/IRTsimrel/articles/case-studies.md)
  — Three complete publication-ready simulation templates
