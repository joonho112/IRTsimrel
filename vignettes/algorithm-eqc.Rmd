...
1. **Bracketed root**: For the `"info"` metric, EQC verifies a sign-changing
   bracket before calling `uniroot()`. If a very wide finite quadrature
   interval causes the upper endpoint to saturate, EQC checks for an interior
   maximum before declaring the target infeasible.
...
$\tilde{\rho}(c^*) = \rho^*$ and let $\hat{c}^*_M$ be the EQC solution with
...
2. **Uniqueness of the root**: On the population target interval described in
   Proposition 1 in `vignette("theory-reliability")`, the equation
   $\rho(c) = \rho^*$ has a unique solution $c^*$ when
   $\rho(c_L) < \rho^* < \rho(c_U)$. The finite empirical implementation also
   verifies the bracket actually used by `uniroot()`.
...
- Stable on practical EQC search intervals; for very wide finite empirical
  quadrature bounds, EQC checks an interior maximum before declaring infeasibility.
