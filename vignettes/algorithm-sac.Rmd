...
When both algorithms use the same reliability metric and comparable design
inputs, and the SAC run has converged, agreement within 5% is typical and
expected. `compare_eqc_sac()` checks target, metric, model, item count, and
status fields, but it cannot prove that every latent/item generator argument or
resampling choice was identical.
