...
Implements Algorithm 1 (EQC/SQC) from Lee (2025). Given a target marginal
...
| `item_source` | character | `"parametric"` | Source passed to `sim_item_params()` (e.g., `"parametric"`, `"irw"`). |
...
provides an independent validation of EQC and can target the exact MSEM-based
...
| `sac_result` | `sac_result` | *required* | Output from `sac_calibrate()`. |
...
```{r compare-shapes, eval=TRUE, fig.cap="Comparison of latent distribution shapes."}
p <- compare_shapes(
  n      = 2000,
...
Computes the MSEM-based marginal reliability using the harmonic mean
of test information.
...
```{r rho-curve-plot, eval=TRUE, fig.cap="Reliability curve for a 25-item Rasch test."}
...
using a different ability sample.
...
```{r plot-sac-traj, eval=TRUE, fig.cap="SAC convergence trajectory (scaling factor)."}
plot(sac_res, type = "c")
```

```{r plot-sac-rho, eval=TRUE, fig.cap="SAC reliability estimates across iterations."}
...
```{r plot-latentg, eval=TRUE, fig.cap="Bimodal latent ability distribution."}
...
```{r plot-items-scatter, eval=TRUE, fig.cap="Difficulty vs. discrimination scatter plot."}
...
```{r workflow-rho-curve, eval=TRUE, fig.cap="Reliability curve for the workflow example."}
rc_wf <- rho_curve(
...
The table below lists every symbol exported from IRTsimrel (from
the NAMESPACE file), organized alphabetically.
