...
**IRTsimrel** v0.2.0. Functions are organized into five categories:
...
parameters, then solves the scalar root-finding problem via Brent's method
(`uniroot()`). This is the recommended starting point for most simulation
studies and targets the bracket-checked average-information metric.
...
| `tol` | numeric | `1e-4` | Tolerance for `uniroot()`. |
...
| `discrimination_params` | list | `list()` | For 2PL: `mu_log` (default 0), `sigma_log` (default 0.3), `rho` (default -0.3). |
| `hierarchical_params` | list | `list()` | For hierarchical: `mu` (2-vector), `tau` (2-vector), `rho`. |
| `custom_params` | list | `list()` | For custom: `beta` (vector or function), `lambda` (vector or function). |
...
red line shows the N(0, 1) reference density.
...
Screens whether a target reliability is achievable for a given test design
by computing the range of reliabilities across a range of scaling factors.
...
A data frame of class `"rho_curve"` (returned invisibly when `plot = TRUE`)
with columns `c`, `rho_tilde`, and/or `rho_bar` depending on the `metric`
argument.
...
reliability estimates. This function requires the **TAM** package to be
installed.
...
For 2PL models, creates difficulty vs. discrimination scatter plots and/or
density plots. Uses **ggplot2** if available.
