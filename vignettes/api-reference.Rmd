...
This vignette provides a comprehensive reference for every exported function in
**IRTsimrel** v0.3.0. Functions are organized into five categories:

1. **Calibration Functions** -- The core algorithms for reliability-targeted
   simulation (`eqc_calibrate`, `sac_calibrate`, `compare_eqc_sac`).
2. **Simulation Functions** -- Generators for latent abilities, item parameters,
   and response data (`sim_latentG`, `sim_item_params`, `simulate_response_data`,
   `compare_shapes`).
3. **Reliability Functions** -- Low-level utilities for computing and exploring
   reliability (`compute_rho_bar`, `compute_rho_tilde`, `compute_rho_both`,
   `compute_apc_init`, `check_feasibility`, `rho_curve`,
   `compute_reliability_tam`).
4. **S3 Methods** -- Print, summary, plot, coef, predict, and coercion methods
...
For conceptual introductions, see `vignette("introduction")`. For applied
workflow guidance, see `vignette("applied-guide")`. For the mathematical
theory, see `vignette("theory-reliability")`.
...
### 1.1 `eqc_calibrate()` -- Empirical Quadrature Calibration {#eqc-calibrate}

Implements Algorithm 1 (EQC) from Lee (2026). Given a target marginal
reliability, EQC draws a large fixed quadrature sample of abilities and item
parameters, maps the reliability topology over the requested log-scale
interval, and applies an explicit root-selection policy. Brent's method
(`uniroot()`) locally polishes ordinary sign-changing crossings; boundary,
tangent, plateau, and infeasible best-achievable outcomes follow their
separate contracts. This is the recommended starting point for most simulation
studies and targets the average-information metric.
...
### 1.2 `sac_calibrate()` -- Stochastic Approximation Calibration {#sac-calibrate}
...
can target the MSEM-based reciprocal-information reliability and provides
same-estimand validation of EQC when `reliability_metric = "info"`.
...
### 1.3 `compare_eqc_sac()` -- Compare Calibration Results {#compare-eqc-sac}
...
The comparison warns when metric, model, test length, or calibration status
metadata differ. Use `reliability_metric = "info"` in SAC for same-estimand
validation of EQC.
...
### 2.1 `sim_latentG()` -- Simulate Latent Ability Distributions {#sim-latentg}
...
mean 0 and variance 1, so changes in distributional shape do not
alter the scale.
...
```{r sim-latentg-normal, eval=TRUE}
...
```{r sim-latentg-bimodal, eval=TRUE}
...
```{r sim-latentg-skewed, eval=TRUE}
...
```{r sim-latentg-custom, eval=TRUE}
...
### 2.2 `sim_item_params()` -- Simulate Item Parameters {#sim-item-params}

Generates item difficulty and discrimination parameters for IRT models.
Supports multiple sources (parametric default, optional IRW, hierarchical,
custom) and methods for inducing difficulty--discrimination correlation.
...
```{r sim-items-hierarchical, eval=TRUE}
...
See `vignette("item-parameters")` for a complete discussion of sources
and methods.

```{r sim-items-3pl, eval=TRUE}
...
### 2.3 `simulate_response_data()` -- Generate Response Matrices {#simulate-response-data}

Simulates binary (0/1) item response data using calibrated parameters from
`eqc_calibrate()` or `sac_calibrate()`. This is the final step before
external validation (e.g., with TAM).

The function always uses the normalized item design stored in the calibration
result. For a fixed-form result this is that calibrated form. For an SAC
`item_scope = "item_superpopulation"` result, it is one stored
representative evaluation form: this call does not redraw a new form and does
not itself simulate the aggregate item-superpopulation estimand.
...
`calibration_status` is a scalar status label; `status_flags` is always a
character vector so EQC, SAC, and legacy SPC provenance can be handled uniformly.
The provenance also records `schema_version`, `item_scope`,
`design_signature`, and item-parameter provenance.

#### Examples

```{r simulate-response, eval=TRUE}
...
```{r simulate-response-sac, eval=TRUE}
...
### 2.4 `compare_shapes()` -- Compare Distribution Shapes {#compare-shapes}
...
A `ggplot` object with faceted density plots, one panel per shape. A dashed
red line shows the $N(0,\sigma^2)$ reference density.

#### Example

```{r compare-shapes, eval=requireNamespace("ggplot2", quietly = TRUE), fig.cap="Comparison of latent distribution shapes.", fig.alt="Faceted density plots comparing normal, bimodal, skewed, and heavy-tailed latent distributions."}
p <- compare_shapes(
  n      = 2000,
...
corresponds to the manuscript's MSEM-based reliability $\bar{w}$ and is
selected with `reliability_metric = "msem"` or `"bar"`. The name `rho_bar` is
an API label for $\bar{w}$, not a separate $\bar{\rho}$ estimand.

### 3.1 `compute_rho_bar()` -- MSEM-Based Marginal Reliability {#compute-rho-bar}

Computes the MSEM-based marginal reliability using the reciprocal-information
MSEM convention described in `vignette("theory-reliability")`.
...
A numeric scalar by default. With `return_diagnostics = TRUE`, a list contains
the reliability estimate together with variance, integration, and numerical
...
### 3.2 `compute_rho_tilde()` -- Average-Information Reliability {#compute-rho-tilde}

Computes the average-information reliability using the arithmetic mean
of test information. This is the recommended
metric for EQC.
...
Parameters and return value are identical to `compute_rho_bar()` above; only
the internal formula differs.

#### Example

```{r rho-tilde, eval=TRUE}
...
### 3.3 `compute_rho_both()` -- Both Metrics in a Single Pass {#compute-rho-both}

Computes both reliability metrics from a single set of matrix computations,
avoiding redundant M x I operations.
...
Parameters are identical to `compute_rho_bar()`. The return value is a named
list.
...
### 3.4 `compute_apc_init()` -- Analytic Pre-Calibration {#compute-apc-init}

Computes an initial scaling factor using a closed-form approximation under
Gaussian Rasch assumptions. Used internally by `sac_calibrate()` when
`c_init = NULL`.
...
### 3.5 `check_feasibility()` -- Feasibility Screening {#check-feasibility}

Screens whether a target reliability is attainable in one generated finite
Monte Carlo design by computing the empirical reliability range across a set
of scaling factors.
Run this before calibration to avoid wasting time on infeasible targets.

The classification is conditional on that finite theta sample. For
`latent_shape = "heavy_tail"`, a numeric MSEM range is only a finite-grid
sensitivity result: the corresponding population MSEM functional can be
non-integrable, so this function does not certify population feasibility.
...
### 3.6 `rho_curve()` -- Reliability as a Function of Scaling Factor {#rho-curve}

Computes and optionally plots a finite-sample reliability curve across a grid
of scaling factor values. It helps visualize the relationship between
discrimination strength and measurement precision for that generated design.
With `latent_shape = "heavy_tail"`, an MSEM curve is finite-grid sensitivity
evidence only and does not establish that the population MSEM functional is
finite.
...
A data frame of class `"rho_curve"`, always returned invisibly, with
columns `c`, `rho_tilde`, and/or `rho_bar` depending on the
`metric` argument. When `plot = TRUE`, the plot is drawn as a side effect.

#### Example

```{r rho-curve-plot, eval=TRUE, fig.cap="Reliability curve for a 25-item Rasch test.", fig.alt="Reliability curve showing average-information and MSEM metrics over scaling factors for a 25-item Rasch test."}
...
### 3.7 `compute_reliability_tam()` -- TAM-Based Validation {#compute-reliability-tam}

Fits a Rasch or 2PL model using the **TAM** package and returns WLE and EAP
reliability estimates. This public helper intentionally does not accept
`model = "3pl"`. Direct Phase 7 validation showed that TAM's 3PL fit can
provide EAP output, whereas `tam.wle()` was unsupported for the validated
`tam.mml.3pl` object. This function requires **TAM** to be installed.
...
TAM coefficients are estimator-, prior-, sample-, and fit-dependent external
diagnostics. They are not identical to IRTsimrel's analytic population
reliability estimands, so agreement is evidence about the workflow rather than
an identity that every finite sample must satisfy.

---

## 4. S3 Methods

IRTsimrel defines S3 methods for six object classes. This section documents
each method with its signature and a brief example.
...
Displays calibration summary, design parameters, convergence diagnostics, and
parameter summaries. Returns `x` invisibly.

```{r print-eqc, eval=TRUE}
print(eqc_res)
```
...
Returns an object of class `"summary.eqc_result"` containing a compact subset
of key results.

```{r summary-eqc, eval=TRUE}
s <- summary(eqc_res)
s
```
...
`lambda_scaled`, and `c_star`; 3PL results also include `guessing`.

```{r coef-eqc, eval=TRUE}
...
If `newdata` is `NULL`, returns `achieved_rho`. If `newdata` is a numeric
vector of scaling factor values, computes reliability at each value using the
stored quadrature sample.

```{r predict-eqc, eval=TRUE}
...
Like `predict.eqc_result()`, but accepts an optional `theta_vec` argument for
using a different ability sample. When `theta_vec` is supplied, prediction uses
that sample's own sample variance, `stats::var(theta_vec)`, as the `theta_var`
basis for reliability recomputation rather than `object$theta_var`.

```{r predict-sac, eval=TRUE}
predict(sac_res)
...
```{r plot-sac-traj, eval=TRUE, fig.cap="SAC convergence trajectory (scaling factor).", fig.alt="Line plot of SAC scaling factor iterates across stochastic approximation iterations."}
plot(sac_res, type = "c")
```

```{r plot-sac-rho, eval=TRUE, fig.cap="SAC reliability estimates across iterations.", fig.alt="Line plot of SAC reliability estimates across iterations with the target reliability as reference."}
...
Plots the latent distribution as histogram, density, or both. Uses **ggplot2**
if available.
...
```{r plot-latentg, eval=TRUE, fig.cap="Bimodal latent ability distribution.", fig.alt="Histogram and density plot of a bimodal latent ability distribution."}
...
```{r as-numeric-latentg, eval=TRUE}
...
`as.double()` is registered as the same extraction path, so
`as.double(g_norm)` also returns the stored theta vector.
...
For 2PL and 3PL models, creates difficulty-versus-discrimination scatter
plots and/or density plots; the 3PL density view also includes the guessing
distribution. Rasch output uses the applicable difficulty/discrimination
summaries. Uses **ggplot2** if available.

| Parameter | Default | Description |
|:----------|:--------|:------------|
| `type` | `"scatter"` | `"scatter"`, `"density"`, or `"both"`. |

```{r plot-items-scatter, eval=TRUE, fig.cap="Difficulty vs. discrimination scatter plot.", fig.alt="Scatter plot of item difficulty against scaled discrimination for a 2PL item set."}
...
```{r print-summary-items, eval=TRUE}
...
```{r print-summary-latentg, eval=TRUE}
...
These functions are retained for backward compatibility and will be removed
in a future release. They issue a deprecation warning when called.

### 5.1 `spc_calibrate()` {#spc-calibrate}

```r
spc_calibrate(...)
```

Deprecated alias for `sac_calibrate()`. All arguments are passed through
unchanged. Use `sac_calibrate()` for new code.

```{r spc-deprecated, eval=FALSE}
...
### 5.2 `compare_eqc_spc()` {#compare-eqc-spc}

```r
compare_eqc_spc(...)
```

Deprecated alias for `compare_eqc_sac()`. All arguments are passed through
unchanged. Use `compare_eqc_sac()` for new code.

```{r compare-deprecated, eval=FALSE}
...
The table below maps each function to the vignette that covers it in detail.

| Function | Category | Primary Vignette |
|:---------|:---------|:-----------------|
| `eqc_calibrate()` | Calibration | `vignette("algorithm-eqc")` |
| `sac_calibrate()` | Calibration | `vignette("algorithm-sac")` |
| `compare_eqc_sac()` | Calibration | `vignette("applied-guide")` |
| `sim_latentG()` | Simulation | `vignette("latent-distributions")` |
| `compare_shapes()` | Simulation | `vignette("latent-distributions")` |
| `sim_item_params()` | Simulation | `vignette("item-parameters")` |
| `simulate_response_data()` | Simulation | `vignette("applied-guide")` |
| `compute_rho_bar()` | Reliability | `vignette("theory-reliability")` |
| `compute_rho_tilde()` | Reliability | `vignette("theory-reliability")` |
| `compute_rho_both()` | Reliability | `vignette("theory-reliability")` |
| `compute_apc_init()` | Reliability | `vignette("algorithm-sac")` |
| `check_feasibility()` | Reliability | `vignette("simulation-design")` |
| `rho_curve()` | Reliability | `vignette("simulation-design")` |
| `compute_reliability_tam()` | Validation | `vignette("validation")` |
| `spc_calibrate()` | Deprecated | -- |
| `compare_eqc_spc()` | Deprecated | -- |
...
```{r workflow-feasibility, eval=TRUE}
...
```{r workflow-rho-curve, eval=TRUE, fig.cap="Reliability curve for the workflow example.", fig.alt="Reliability curve across scaling factors for the complete workflow example."}
rc_wf <- rho_curve(
...
The table below lists the user-facing functions exported by IRTsimrel plus
registered S3 methods from the NAMESPACE file, organized alphabetically.
