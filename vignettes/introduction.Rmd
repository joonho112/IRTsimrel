...
Warehouse integration when available, or user-supplied custom parameters) and
are preserved exactly throughout calibration.
...
It also carries provenance metadata describing the metric, calibration status,
item source, design, and simulation seed. You can use the response matrix
...
well behaved on practical EQC search intervals. The implementation verifies
the bracket before root-finding so that
...
| Function | Purpose |
|----------|---------|
| `eqc_calibrate()` | Algorithm 1: deterministic calibration via Brent's root-finding method for average-information reliability |
...
- **SAC warm start from EQC**: Pass an `eqc_result` object directly to
  `sac_calibrate(c_init = eqc_result, reliability_metric = "info")` for fast,
  well-initialized same-estimand stochastic validation.
...
The `target_status_info` and `target_status_msem` fields report whether the
requested target is below, within, on the boundary of, or above the achievable
