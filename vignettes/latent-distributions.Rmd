...
description: >
  ...
  sim_latentG(), covering 12 supported shapes including custom mixtures, covariate
...
2. Available distribution shapes (12 supported shapes including `custom`)
...
- The `sigma` parameter directly controls the residual standard deviation;
  covariate variation can increase the marginal SD of `theta`
...
`sim_latentG()` provides 12 supported shapes, including 11 built-in
pre-standardized shapes plus the `custom` mixture option.

### 4.1 Standard Normal

The baseline case: $z \sim N(0, 1)$

```{r normal, fig.alt = "Histogram and density plot of a standard normal latent ability distribution with a normal reference curve."}
...
```{r bimodal, fig.alt = "Histogram and density plot of a bimodal latent ability distribution with two symmetric modes."}
...
```{r bimodal-delta, fig.alt = "Histogram and density plot of a bimodal latent distribution with wider mode separation."}
# Wider separation
...
```{r trimodal, fig.alt = "Histogram and density plot of a trimodal latent ability distribution with one central and two side modes."}
...
```{r trimodal-custom, fig.alt = "Histogram and density plot of a trimodal latent distribution with a stronger central peak."}
...
```{r multimodal, fig.alt = "Histogram and density plot of a four-component multimodal latent ability distribution."}
...
```{r skew-pos, fig.alt = "Histogram and density plot of a positively skewed latent ability distribution."}
...
```{r skew-neg, fig.alt = "Histogram and density plot of a negatively skewed latent ability distribution."}
...
The built-in Student-$t$ heavy-tail population is suitable for the
average-information metric, but not for population MSEM targeting under the
reciprocal-information approximation: the expectation of inverse information
diverges in the tails. A finite Monte Carlo sample can still return a finite
`rho_bar`, but that number is a sample/grid diagnostic rather than a finite
population estimand. SAC rejects this incompatible population-MSEM contract.

```{r heavy-tail, fig.alt = "Histogram and density plot of a heavy-tailed latent ability distribution with a normal reference curve."}
...
```{r light-tail, fig.alt = "Histogram and density plot of a light-tailed latent ability distribution."}
...
```{r uniform, fig.alt = "Histogram and density plot of a uniform latent ability distribution."}
...
```{r floor, fig.alt = "Histogram and density plot of a latent ability distribution with a floor effect."}
...
```{r ceiling, fig.alt = "Histogram and density plot of a latent ability distribution with a ceiling effect."}
...
```{r compare-shapes, fig.width = 9, fig.height = 7, eval = requireNamespace("ggplot2", quietly = TRUE), fig.alt = "Faceted density comparison across all built-in latent distribution shapes."}
compare_shapes(
  n = 3000,
...
```{r custom-mixture, fig.alt = "Histogram and density plot of a custom three-component latent mixture distribution."}
...
In Rasch, 2PL, and 3PL models, the latent distribution $G$ affects key
quantities:
...
Different latent shapes lead to different empirical achievable-reliability
ranges for the same realized design and Monte Carlo grid. This section uses
`check_feasibility()` and `rho_curve()` to explore that connection. Treat the
reported MSEM range for `heavy_tail` as a finite-grid sensitivity diagnostic,
not as a finite population-MSEM quantity.
...
```{r shape-curves, fig.height = 5.5, fig.alt = "Reliability curves comparing how latent distribution shape changes the reliability-scale relationship."}
...
Choose quadrature size and replication sample size through a sensitivity or
Monte Carlo precision analysis. Heavier tails and sparse mixture components
often require larger draws, but no universal `M` or `n` threshold guarantees a
desired error. Report the values used and, for calibration claims, evaluate the
selected scale on independent theta draws.
