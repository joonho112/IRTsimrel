...
Empirical Quadrature Calibration (EQC) is the **primary algorithm** in IRTsimrel for reliability-targeted simulation. Given a target marginal reliability $\rho^*$, EQC finds a global discrimination scaling factor $c^*$ such that the population reliability equals the target.
...
For most simulation studies, EQC is all you need. Use SPC (Algorithm 2) only for validation or specialized applications.

## Mathematical Foundation

### The Inverse Problem

**Forward problem**: Given item parameters and a scaling factor $c$, compute reliability $\rho(c)$.

**Inverse problem**: Given target reliability $\rho^*$, find $c^*$ such that $\rho(c^*) = \rho^*$.

### Discrimination Scaling

EQC solves the inverse problem by scaling all item discriminations by a common factor:

$$\lambda_i(c) = c \cdot \lambda_i^{(0)}$$

where $\lambda_i^{(0)}$ are baseline discriminations from the item generator.

**Why this works**:

1. **Separates informativeness from structure**: Changing $c$ modifies reliability without altering item difficulty distributions or latent trait shape
2. **Monotonic relationship**: Reliability is strictly increasing in $c$
3. **Reduces dimensionality**: Multi-dimensional design space becomes a 1D root-finding problem

### Monotonicity Guarantee

**Key Property**: Under mild regularity conditions, $\rho(c)$ is strictly monotonically increasing in $c$:
...
- **Existence and uniqueness**: For any $\rho^* \in (0, 1)$, there exists a unique $c^*$
...
Draw large fixed samples from the latent and item parameter distributions:

- $\{\theta_m\}_{m=1}^M \sim G$ (latent abilities)
- $\{(\beta_i, \lambda_i^{(0)})\}_{i=1}^I \sim H$ (item parameters)

These samples serve as an **empirical quadrature rule** for approximating population expectations.
...
This approximates the population reliability $\mathbb{E}[\rho(c)]$ by the Law of Large Numbers.
...
using Brent's method (`uniroot()` in R), which combines bisection, secant, and inverse quadratic interpolation for robust convergence.
...
```{r target-rho-examples, eval=FALSE}
...
The number of latent ability samples for empirical integration. Larger $M$ reduces Monte Carlo error:
...
**Recommendation**: $M \geq 10,000$ for routine use; $M \geq 50,000$ for high-precision work.
...
The `info` metric typically requires a **smaller** $c^*$ to achieve the same target, because it yields higher reliability values for a given $c$.

**Recommendation**:

- Use `"msem"` (default) for theoretical correctness
- Use `"info"` when targeting very high reliability that `"msem"` cannot achieve
...
Different latent shapes require different $c^*$ values to achieve the same target reliability:

```{r compare-shapes-effect, eval=FALSE}
...
An important validation finding: TAM's EAP reliability is systematically **higher** than WLE reliability. This is not a bug but reflects different mathematical definitions:
...
When the target reliability exceeds what's achievable with the given configuration:

```{r boundary-high, warning=TRUE}
...
```
The function returns `c_star = c_bounds[2]` (upper bound) with a warning.
...
The function returns `c_star = c_bounds[1]` (lower bound) with a warning.
...
When EQC and SPC agree (typically within 5%), you can be confident in your calibration.
...
In practice, convergence to machine precision typically requires < 50 function evaluations.
...
With $M = 10,000$, the Monte Carlo standard error is approximately 0.01.

### Consistency

As $M \to \infty$, the EQC solution converges to the true population solution:

$$\hat{c}^*_M \xrightarrow{\text{a.s.}} c^* \quad \text{as } M \to \infty$$

## Summary

EQC is a fast, accurate, and reliable algorithm for reliability-targeted IRT simulation:
...
For specialized applications or independent validation, see the SPC algorithm vignette.

## References
Lee, J.-H. (2025). Reliability-targeted simulation for item response data. *Manuscript in preparation*.
