...
describe("Custom source for item parameters", {
  ...
  test_that("custom with exact beta and lambda vectors", {
    ...
    items <- expect_no_warning(sim_item_params(
    ...
    expect_true(is.na(items$achieved$overall$cor_spearman_pooled))
    expect_true(is.na(items$achieved$by_form[[1]]$cor_pearson))
    expect_true(is.na(items$achieved$by_form[[1]]$cor_spearman))
  })
...
describe("S3 methods for item_params", {
  ...
  test_that("print.item_params runs without error and returns invisibly", {
    ...
    out <- NULL
    capture.output(out <- expect_invisible(print(items)))
...
describe("Input validation for sim_item_params", {
  ...
  test_that("logical and parameter list controls are validated before generation", {
    expect_error(
      sim_item_params(n_items = 10, center_difficulties = NA),
      "`center_difficulties` must be TRUE or FALSE"
    )
    expect_error(
      sim_item_params(n_items = 10, difficulty_params = "bad"),
      "`difficulty_params` must be a list"
    )
    expect_error(
      sim_item_params(n_items = 10, difficulty_params = list(NA_real_)),
      "`difficulty_params` entries must all be named"
    )
  })

  test_that("parametric difficulty parameters reject nonfinite values", {
    expect_error(
      sim_item_params(
        n_items = 10, source = "parametric",
        difficulty_params = list(distribution = "gamma")
      ),
      "difficulty_params\\$distribution"
    )
    expect_error(
      sim_item_params(
        n_items = 10, source = "parametric",
        difficulty_params = list(mu = Inf)
      ),
      "difficulty_params\\$mu"
    )
    expect_error(
      sim_item_params(
        n_items = 10, source = "parametric",
        difficulty_params = list(sigma = 0)
      ),
      "difficulty_params\\$sigma"
    )
  })

  test_that("discrimination parameters reject invalid scalars", {
    expect_error(
      sim_item_params(
        n_items = 10, model = "2pl", source = "parametric",
        discrimination_params = list(mu_log = Inf)
      ),
      "discrimination_params\\$mu_log"
    )
    expect_error(
      sim_item_params(
        n_items = 10, model = "2pl", source = "parametric",
        discrimination_params = list(sigma_log = -1)
      ),
      "discrimination_params\\$sigma_log"
    )
    expect_error(
      sim_item_params(
        n_items = 10, model = "2pl", source = "parametric",
        discrimination_params = list(rho = NA_real_)
      ),
      "discrimination_params\\$rho"
    )
  })

  test_that("hierarchical 2PL parameters reject malformed covariance inputs", {
    expect_error(
      sim_item_params(
        n_items = 10, model = "2pl", source = "hierarchical",
        hierarchical_params = list(mu = c(0, Inf))
      ),
      "hierarchical_params\\$mu"
    )
    expect_error(
      sim_item_params(
        n_items = 10, model = "2pl", source = "hierarchical",
        hierarchical_params = list(tau = c(0.3, -1))
      ),
      "hierarchical_params\\$tau"
    )
    expect_error(
      sim_item_params(
        n_items = 10, model = "2pl", source = "hierarchical",
        hierarchical_params = list(rho = 1)
      ),
      "hierarchical_params\\$rho"
    )
  })
  ...
  test_that("custom functions must return finite vectors of the right shape", {
    expect_error(
      sim_item_params(
        n_items = 5, model = "rasch", source = "custom",
        custom_params = list(beta = function(n) rep(0, n - 1L))
      ),
      "custom_params\\$beta"
    )
    expect_error(
      sim_item_params(
        n_items = 5, model = "rasch", source = "custom",
        custom_params = list(beta = function(n) c(rep(0, n - 1L), Inf))
      ),
      "custom_params\\$beta"
    )
    expect_error(
      sim_item_params(
        n_items = 5, model = "2pl", source = "custom",
        custom_params = list(beta = rep(0, 5), lambda = c(1, 1, 0, 1, 1))
      ),
      "custom_params\\$lambda"
    )
  })
})
