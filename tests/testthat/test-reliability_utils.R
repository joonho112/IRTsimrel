...
# ---------------------------------------------------------------------------
# Jensen's inequality: rho_tilde >= rho_bar on the same information grid
# ---------------------------------------------------------------------------

test_that("internal reliability engine matches public wrappers with and without eta_base", {
  set.seed(42)
  theta <- rnorm(500)
  beta <- rnorm(12)
  lambda <- exp(rnorm(12, 0, 0.2))
  theta_var <- var(theta)
  eta_base <- outer(theta, beta, "-")

  for (c_val in c(0.3, 1.0, 2.5)) {
    rho_tilde <- compute_rho_tilde(c_val, theta, beta, lambda, theta_var = theta_var)
    rho_bar <- compute_rho_bar(c_val, theta, beta, lambda, theta_var = theta_var)

    expect_equal(
      IRTsimrel:::.compute_rho_generic(
        c_val, theta, beta, lambda,
        theta_var = theta_var,
...
test_that("compute_rho_both uses the same engine as single-metric wrappers", {
  set.seed(7)
  theta <- rnorm(700)
  beta <- rnorm(10)
  lambda <- exp(rnorm(10, 0, 0.3))
  theta_var <- var(theta)

  both <- compute_rho_both(1.4, theta, beta, lambda, theta_var = theta_var)

  expect_equal(
    both$rho_tilde,
    compute_rho_tilde(1.4, theta, beta, lambda, theta_var = theta_var),
    tolerance = 1e-12
  )
  expect_equal(
    both$rho_bar,
    compute_rho_bar(1.4, theta, beta, lambda, theta_var = theta_var),
    tolerance = 1e-12
  )
})

test_that("precomputed eta_base dimensions are validated", {
  set.seed(1)
  theta <- rnorm(20)
  beta <- rnorm(5)
  lambda <- rep(1, 5)
  ...
  expect_error(
    IRTsimrel:::.compute_rho_generic(
      1, theta, beta, lambda,
      metric_internal = "info",
      eta_base = matrix(0, nrow = 19, ncol = 5)
    ),
    "eta_base"
  )
})

test_that("rho_curve metrics share the same reliability engine", {
  c_values <- c(0.5, 1, 1.5, 2)
  ...
  expect_equal(both$rho_tilde, info$rho_tilde, tolerance = 1e-12)
  expect_equal(both$rho_bar, msem$rho_bar, tolerance = 1e-12)
})
...
test_that("rho_bar may decrease at very large c (known non-monotone behavior)", {
  ...
  expect_lt(rho_bar_extreme, rho_bar_moderate)
  expect_true(rho_bar_extreme < 1)
  expect_true(rho_bar_extreme >= 0)
})

test_that("MSEM feasibility can be interior even when endpoints are below target", {
  ...
  expect_lt(feas$rho_bounds_msem["lower"], 0.75)
  expect_lt(feas$rho_bounds_msem["upper"], 0.75)
  expect_gt(feas$rho_range_msem["upper"], 0.75)
  expect_equal(feas$target_status_msem, "feasible")
})
...
describe("compute_apc_init()", {
  ...
  test_that("APC init rejects invalid inputs", {
    expect_error(compute_apc_init(0, 25), "`target_rho`")
    expect_error(compute_apc_init(1, 25), "`target_rho`")
    expect_error(compute_apc_init(1.5, 25), "`target_rho`")
    expect_error(compute_apc_init(0.80, 0), "`n_items`")
    expect_error(compute_apc_init(0.80, 25, sigma_beta = -1), "`sigma_beta`")
    expect_error(compute_apc_init(0.80, 25, sigma_beta = Inf), "`sigma_beta`")
  })
