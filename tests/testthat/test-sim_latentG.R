...
describe("Input validation errors in sim_latentG()", {
  ...
  test_that("n=NA and n=Inf errors cleanly", {
    expect_error(sim_latentG(n = NA_real_), "`n` must be a positive integer")
    expect_error(sim_latentG(n = Inf), "`n` must be a positive integer")
  })

  test_that("sigma=-1 errors", {
    expect_error(sim_latentG(n = 10, sigma = -1), "`sigma` must be a positive finite scalar")
  })

  test_that("sigma=0 errors", {
    expect_error(sim_latentG(n = 10, sigma = 0), "`sigma` must be a positive finite scalar")
  })

  test_that("sigma=Inf errors", {
    expect_error(sim_latentG(n = 10, sigma = Inf), "`sigma` must be a positive finite scalar")
  })

  test_that("sigma=NaN errors", {
    expect_error(sim_latentG(n = 10, sigma = NaN), "`sigma` must be a positive finite scalar")
  })
  ...
  test_that("floor and ceiling shape parameters validate cleanly", {
    expect_error(
      sim_latentG(n = 10, shape = "floor", shape_params = list(w_floor = -0.2)),
      "w_floor"
    )
    expect_error(
      sim_latentG(n = 10, shape = "floor", shape_params = list(w_floor = 1)),
      "w_floor"
    )
    expect_error(
      sim_latentG(n = 10, shape = "floor", shape_params = list(m_floor = Inf)),
      "m_floor"
    )
    expect_error(
      sim_latentG(n = 10, shape = "ceiling", shape_params = list(w_ceil = -0.2)),
      "w_ceil"
    )
    expect_error(
      sim_latentG(n = 10, shape = "ceiling", shape_params = list(w_ceil = 1)),
      "w_ceil"
    )
    expect_error(
      sim_latentG(n = 10, shape = "ceiling", shape_params = list(m_ceil = Inf)),
      "m_ceil"
    )
  })
  ...
  test_that("custom mixture_spec components must be finite numeric vectors", {
    expect_error(
      sim_latentG(
        n = 10, shape = "custom",
        mixture_spec = list(
          weights = c(0.5, NA), means = c(0, 1), sds = c(1, 1)
        )
      ),
      "mixture_spec\\$weights"
    )
    expect_error(
      sim_latentG(
        n = 10, shape = "custom",
        mixture_spec = list(
          weights = c(0.5, -0.5), means = c(0, 1), sds = c(1, 1)
        )
      ),
      "mixture_spec\\$weights"
    )
    expect_error(
      sim_latentG(
        n = 10, shape = "custom",
        mixture_spec = list(
          weights = c(0.5, 0.5), means = c(0, Inf), sds = c(1, 1)
        )
      ),
      "mixture_spec\\$means"
    )
    expect_error(
      sim_latentG(
        n = 10, shape = "custom",
        mixture_spec = list(
          weights = c(0.5, 0.5), means = c(0, 1), sds = c(1, NA)
        )
      ),
      "mixture_spec\\$sds"
    )
  ...
  test_that("logical output controls reject NA", {
    expect_error(
      sim_latentG(n = 10, shape = "custom",
                  mixture_spec = list(weights = 1, means = 0, sds = 1),
                  standardize_custom = NA),
      "`standardize_custom` must be TRUE or FALSE"
    )
    expect_error(
      sim_latentG(n = 10, return_z = NA),
      "`return_z` must be TRUE or FALSE"
    )
  })
