...
test_that("check_feasibility validates inputs", {
  ...
  expect_error(check_feasibility(n_items = 20, M = 1, verbose = FALSE),
               "`M` must be at least 2")
  expect_error(check_feasibility(n_items = 20, target_rho = 1, verbose = FALSE),
               "`target_rho`")
})
...
test_that("check_feasibility classifies optional target_rho consistently", {
  feas_ok <- check_feasibility(
    n_items = 20, model = "rasch", target_rho = 0.70,
    seed = 42, M = 2000, verbose = FALSE
  )
  expect_equal(feas_ok$target_status_info, "feasible")
  expect_true(feas_ok$target_status_msem %in% c(
    "below_lower", "boundary_lower", "feasible", "boundary_upper", "above_upper"
  ))
  expect_named(feas_ok$rho_range_info, c("lower", "upper"))
  expect_named(feas_ok$rho_range_msem, c("lower", "upper"))
  expect_named(feas_ok$rho_bounds_msem, c("lower", "upper"))
  expect_true(feas_ok$rho_msem_max_c >= feas_ok$c_bounds[1])
  expect_true(feas_ok$rho_msem_max_c <= feas_ok$c_bounds[2])

  feas_low <- check_feasibility(
    n_items = 50, model = "rasch", target_rho = 0.01,
    c_bounds = c(0.3, 3), seed = 42, M = 2000, verbose = FALSE
  )
  expect_equal(feas_low$target_status_info, "below_lower")

  feas_high <- check_feasibility(
    n_items = 5, model = "rasch", target_rho = 0.99,
    seed = 42, M = 2000, verbose = FALSE
  )
  expect_equal(feas_high$target_status_info, "above_upper")

  feas_boundary_lower <- check_feasibility(
    n_items = 20, model = "rasch", seed = 42, M = 2000,
    verbose = FALSE
  )
  feas_boundary_lower <- check_feasibility(
    n_items = 20, model = "rasch",
    target_rho = feas_boundary_lower$rho_range_info["lower"],
    seed = 42, M = 2000, verbose = FALSE
  )
  expect_equal(feas_boundary_lower$target_status_info, "boundary_lower")
})
...
test_that("check_feasibility info endpoints align with EQC rho_bounds", {
  ...
  expect_equal(
    unname(feas$rho_bounds_info),
    unname(eqc$misc$rho_bounds_named),
    tolerance = 1e-12
  )
})

test_that("check_feasibility uses an interior info maximum for very wide bounds", {
  feas <- check_feasibility(
    n_items = 25,
    model = "rasch",
    item_source = "parametric",
  ...
  expect_equal(feas$target_status_info, "feasible")
  expect_lt(unname(feas$rho_bounds_info["upper"]), 0.80)
  expect_gt(unname(feas$rho_range_info["upper"]), 0.80)
  expect_lt(feas$rho_info_max_c, feas$c_bounds[2])
})
