...
describe("latent_G S3 methods", {
  ...
  test_that("print.latent_G does not error and returns input invisibly", {
    out <- NULL
    capture.output(out <- expect_invisible(print(latent_obj)))
  ...
  test_that("print.latent_G accepts digits and returns input invisibly", {
    out <- NULL
    capture.output(out <- expect_invisible(print(latent_obj, digits = 2)))
    expect_identical(out, latent_obj)
    output <- capture.output(print(latent_obj, digits = 2))
    expect_true(any(grepl("Target mu : 0\\.00", output)))
  })
  ...
  test_that("as.numeric/as.double latent_G methods return theta vector", {
    expect_identical(as.numeric(latent_obj), latent_obj$theta)
    expect_identical(as.double(latent_obj), latent_obj$theta)
    expect_identical(as.numeric.latent_G(latent_obj), latent_obj$theta)
  })
  ...
  test_that("plot.latent_G validates bins", {
    expect_error(plot(latent_obj, bins = 0), "bins")
    expect_error(plot(latent_obj, bins = 1.5), "bins")
    expect_error(plot(latent_obj, bins = NA_real_), "bins")
  })
...
describe("item_params S3 methods", {
  ...
  test_that("print.item_params does not error and returns invisibly", {
    out <- NULL
    capture.output(out <- expect_invisible(print(item_obj)))
  ...
  test_that("print.item_params accepts digits and returns input invisibly", {
    out <- NULL
    capture.output(out <- expect_invisible(print(item_obj, digits = 2)))
    expect_identical(out, item_obj)
    output <- capture.output(print(item_obj, digits = 2))
    expect_true(any(grepl("Scale factor   : 1\\.00", output)))
  })
  ...
  test_that("as.data.frame.item_params honors row.names", {
    custom_names <- paste0("row", seq_len(nrow(item_obj$data)))
    df <- as.data.frame(item_obj, row.names = custom_names)
    expect_equal(row.names(df), custom_names)

    expect_error(
      as.data.frame(item_obj, row.names = custom_names[-1]),
      "row.names"
    )
  })
...
describe("eqc_result S3 methods", {
  ...
  test_that("print.eqc_result does not error and returns invisibly", {
    out <- NULL
    capture.output(out <- expect_invisible(print(eqc_obj)))
  ...
  test_that("print.eqc_result produces informative output", {
    ...
    expect_true(any(grepl("Empirical Quadrature Calibration|EQC", output,
                          ignore.case = TRUE)))
    expect_true(any(grepl("Average-information \\(tilde\\)", output)))
    expect_false(any(grepl("MSEM-based", output)))
  })
  ...
  test_that("print.summary.eqc_result does not error and returns invisibly", {
    s <- summary(eqc_obj)
    out <- NULL
    capture.output(out <- expect_invisible(print(s)))
  ...
  test_that("print.summary.eqc_result validates summary schema", {
    s <- summary(eqc_obj)
    s$theta_var <- NULL
    expect_error(print(s), "missing required field.*theta_var")
  })

  test_that("EQC S3 methods validate malformed same-class objects", {
    bad_coef <- eqc_obj
    bad_coef$lambda_scaled <- bad_coef$lambda_scaled[-1]
    expect_error(coef(bad_coef), "same length")

    bad_summary <- eqc_obj
    bad_summary$misc$root_status <- NULL
    expect_error(summary(bad_summary), "root_status")

    bad_predict <- eqc_obj
    bad_predict$theta_quad <- NULL
    expect_error(predict(bad_predict, newdata = 1), "theta_quad")

    bad_metric <- eqc_obj
    bad_metric$metric <- "tilde"
    expect_error(print(bad_metric), "canonical stored metric")
  })

  test_that("predict.eqc_result validates newdata without coercion", {
    bad_newdata <- list("1", list(1), numeric(0), NA_real_, NaN, Inf, c(-1, 1))
    for (value in bad_newdata) {
      expect_error(predict(eqc_obj, newdata = value), "newdata")
    }
  })
})
...
describe("sac_result S3 methods", {
  ...
  test_that("print.sac_result does not error and returns invisibly", {
    out <- NULL
    capture.output(out <- expect_invisible(print(sac_obj)))
  ...
  test_that("summary.sac_result returns object of class 'summary.sac_result'", {
    ...
    expect_equal(s$theta_var, sac_obj$theta_var)
  })
  ...
  test_that("print.summary.sac_result validates summary schema", {
    s <- summary(sac_obj)
    s$theta_var <- NULL
    expect_error(print(s), "missing required field.*theta_var")
  })
  ...
  test_that("plot.sac_result type='trajectory' aliases type='c'", {
    skip_if_not_installed("ggplot2")
    p <- plot(sac_obj, type = "trajectory")
    expect_s3_class(p, "gg")
  })
  ...
  test_that("SAC S3 methods validate malformed same-class objects", {
    bad_summary <- sac_obj
    bad_summary$convergence <- NULL
    expect_error(summary(bad_summary), "convergence")

    bad_plot <- sac_obj
    bad_plot$trajectory <- bad_plot$trajectory[-1]
    expect_error(plot(bad_plot, type = "c"), "trajectory")

    bad_coef <- sac_obj
    bad_coef$items_calib$data$lambda[1] <-
      bad_coef$items_calib$data$lambda[1] * 2
    expect_error(coef(bad_coef), "items_calib\\$data\\$lambda")
  })

  test_that("predict.sac_result validates newdata and theta_vec", {
    bad_newdata <- list("1", list(1), numeric(0), NA_real_, NaN, Inf, c(-1, 1))
    for (value in bad_newdata) {
      expect_error(predict(sac_obj, newdata = value), "newdata")
    }
    expect_error(predict(sac_obj, newdata = 1, theta_vec = c(NA_real_, 1)),
                 "theta_vec")
    expect_error(predict(sac_obj, newdata = 1, theta_vec = 1),
                 "theta_vec")
    expect_error(predict(sac_obj, newdata = 1, theta_vec = rep(0, 4)),
                 "positive variance")
  })
